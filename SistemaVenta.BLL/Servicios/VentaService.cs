...
namespace SistemaVenta.BLL.Servicios
{
    public class VentaService : IVentaService
    {
        ...
        {
            //se realiza una consulta antes de buscar el historial
            IQueryable<Venta> query = await _ventaRepositorio.Consultar();
            var ListaResultado = new List<Venta>();

            try
            {
                //validamos la busqueda por fecha
                if(buscarPor == "fecha")
                {
                    DateTime fecha_Inicio = DateTime.ParseExact(fechaInicio, "dd/MM/yyy", new CultureInfo("es-ARG"));
                    DateTime fecha_Fin = DateTime.ParseExact(fechaInicio, "dd/MM/yyy", new CultureInfo("es-ARG"));
